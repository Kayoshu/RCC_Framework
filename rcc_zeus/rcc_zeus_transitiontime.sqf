 ...
 * Date change during 5 sec fadetoblack, optional show new date with GPS grid
 * 
 * Arguments:
 * No Parameters
 *
		...
		[_text1, _timing1, _text2, _timing2, _datechange, _month+1, _day+1, _hours, _minutes, _show], { 
			...
			if (_datechange) then {
				...
				if (_show) then {
					...
					[
						[[format [ "%2 - Grid %1", mapGridPosition player, _hours + "H" + _minutes], "<t align = 'right' size = '0.8'>%1</t>", 64]], safezoneX + safeZoneW / 2, safeZoneH / 2, "<t font='RobotoCondensed'>%1</t>"
					] spawn BIS_fnc_typeText; // display new date & grid
				};
...
[
	...
	[
		...
		["TOOLBOX:YESNO", "Change Date Between texts", [false], true],
